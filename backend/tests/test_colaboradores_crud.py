...
def test_socio_responde_por_areas_e_wizard_recebe(client, as_admin):
    resp = client.post(
        "/api/colaboradores",
        json={"nome": "Gabriel", "email": "gabriel@cf.com", "papel": "socio", "areas": ["Cível", "Tributário"]},
    )
    assert resp.status_code == 201
    assert resp.json()["areas"] == ["Cível", "Tributário"]

    wizard = client.get("/api/users/colaboradores").json()["colaboradores"]
    assert wizard[0]["areas"] == ["Cível", "Tributário"]


def test_area_tem_um_so_socio_responsavel(client, as_admin):
    client.post("/api/colaboradores", json={"nome": "Gabriel", "papel": "socio", "areas": ["Cível"]})
    monica = client.post("/api/colaboradores", json={"nome": "Monica", "papel": "socio"}).json()

    assert client.post(
        "/api/colaboradores", json={"nome": "Outro", "papel": "socio", "areas": ["Cível"]}
    ).status_code == 409
    assert client.patch(f"/api/colaboradores/{monica['id']}", json={"areas": ["Cível"]}).status_code == 409
    # A propria area pode ser regravada pelo mesmo socio
    ok = client.patch(f"/api/colaboradores/{monica['id']}", json={"areas": ["Trabalhista"]})
    assert ok.status_code == 200
    assert client.patch(f"/api/colaboradores/{monica['id']}", json={"areas": ["Trabalhista"]}).status_code == 200


def test_so_socio_responde_por_area(client, as_admin):
    assert client.post(
        "/api/colaboradores", json={"nome": "Gabriela", "papel": "advogado", "areas": ["Trabalhista"]}
    ).status_code == 422
